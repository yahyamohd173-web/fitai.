(()=>{
  ...
  function apply(){
    document.querySelectorAll('.choice-photo').forEach(img=>{
      const name=(img.alt||'').replace(/ fashion example$/,'');
      if(images[name]) img.src=images[name];
    });
    document.querySelectorAll('.trend-photo').forEach(img=>{
      const name=(img.alt||'').replace(/ fashion example$/,'');
      if(images[name]) img.src=images[name];
    });
    ...
    style.textContent=`
      ...
      .trend-grid article>h3,.trend-grid article>p{padding-left:20px;padding-right:20px}
      .trend-grid article>p{padding-bottom:18px}
